...
\begin{itemize}
    \item \textbf{A novel unbiased estimator for bipartite experiments with partial assignment.} We construct an ERL-type estimator that is unbiased under partial assignment~--- \emph{whatever experience the unassigned units receive} (e.g., participation in other, non-overlapping experiments), as long as it enters the expected outcomes additively~--- formally reduces to standard ERL at full assignment, and is provably the minimum-variance member of a natural class of linear estimators. We establish a bias bound without any linearity assumption, and exact unbiasedness, consistency, and asymptotic normality under a linear-contributions model.
...
Like Assumption~A\ref{a:mu}(ii) itself, the second-moment condition holds under the standard randomisation construction, in which the assignment indicators are exogenous to all outcome disturbances and an unassigned unit's would-be assignment is never materialised. The proof (Section~A.2 of the supplementary material) rests on an exact orthogonal decomposition: any admissible $\hat\tau_c$ equals EARL plus a mean-zero term that is uncorrelated with it, so deviating from $c(0)=0$ can only add noise. The guarantee is confined to the class \eqref{eq:class}: a seemingly natural alternative~--- weighting by the product of the \emph{standardised} allocation coverage and the \emph{standardised} full-graph exposure, $\frac{G_a - \E[G_a]}{\var(G_a)}\cdot \frac{H_a - \E[H_a]}{\var(H_a)}$~--- falls outside \eqref{eq:class} owing to its cross terms ($r\neq r'$), so Proposition~\ref{prop:optimality} does not rank it against EARL (see the discussion in the supplementary material). We include this variant (EARL-centred) in the simulation study, where its error exceeds EARL's by 1--2 orders of magnitude on the dense graphs we consider.
...
\caption{RMSE (top, log scale) and bias (bottom) versus the allocation rate $q$ in scenario S1 ($\gamma_u=1$). EARL is unbiased everywhere and has the lowest RMSE at almost every allocation rate; ERL-drop has bias $-(1-q)\cdot GATE$ (dotted line); the IPW benchmarks are erratic or degenerate (IPW-alloc returns zero in every replication on MovieLens, where a pure exposure was never observed); EARL-centred pays a variance penalty growing with graph density. At $q=1$ all ERL-type estimators recover standard ERL (EARL-centred, whose allocation factor is undefined there, is computed with the exposure factor alone).}
\label{fig:rmse-bias}
\end{figure*}
...
We studied bipartite experiments in which only part of the randomisation-unit population is enrolled, a common regime on experimentation platforms. Restricting the analysis to the enrolled units attenuates the estimated global treatment effect by the allocation rate; EARL removes this bias. It is unbiased whatever (additively entering) experience the unassigned units receive, minimum-variance in its class, and consistent and asymptotically normal under the sparsity conditions known for full-assignment ERL, with closed-form and randomisation-inference variance estimators. Simulations on real bipartite graphs confirm the theory, show up to six-fold accuracy gains over the strongest existing baseline, and delineate the low-allocation, high-outcome-level regime where outcome centring is advisable.

Several directions remain open: unit-specific design probabilities, covariate adjustment (CA-ERL \cite{shi2025scalableanalysisbipartiteexperiments}), and designs optimising the allocation split across concurrent experiments \cite{harshaw2023erl,brennan2022cluster}.
